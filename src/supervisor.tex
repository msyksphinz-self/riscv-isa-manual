...
provided.
\end{comment}
~\ref{otherpriv}節で定義したSRET命令に加えて、新しいスーパバイザレベルの命令を提供する。

\begin{comment}
\subsection{Supervisor Memory-Management Fence Instruction}
\end{comment}
\subsection{スーパバイザメモリ管理フェンス命令}
...
スーパバイザメモリ管理フェンス命令SFENCE.VMAは、
メモリ内のメモリ管理データ構造の更新を現在の実行と同期させるために使用されます。
SFENCE.VMA命令を実行すると、これらのデータ構造に対する暗黙のリード/ライトが発生しますが、
これらの暗黙の参照は、通常、明示的なロード/ストア命令に対して順序付けられません。
SFENCE.VMA命令を実行すると、現在のRISC-V hartに既に実行されているストア命令が、
そのhartからメモリ管理データ構造への後続の暗黙の参照の前に順序付けられることが保証されます。
この命令の動作の詳細は、~\ref{virt-control}項および~\ref{pmp-vmem}項に記載されています。
...
\end{comment}
SFENCE.VMAは、アドレス変換に関連するすべてのローカルハードウェアキャッシュをフラッシュするために使用されます。
TLBのフラッシュではなく、フェンスとして仕様化されているのは、
フラッシュ操作によってどの命令が影響を受けるかについて、よりクリーンなセマンティクスを提供し、
より多様なダイナミックキャッシング構造やメモリ管理スキームをサポートするためです。
また、SFENCE.VMAは、ページテーブルの書き込みとアドレス変換ハードウェアの同期をとるために、
より高い権限レベルで使用されます。
\end{commentary}
...
SFENCE.VMAは、ローカルハートのメモリ管理データ構造への暗黙の参照のみを順序付けます。
...
\end{comment}
そのため、メモリ管理データ構造が変更された場合、他のhartには別途通知する必要があります。
1) ローカルデータをフェンスしてローカルの書き込みがグローバルに見えるようにし、
2) 他のスレッドにプロセッサ間割り込みをかけ、
3) リモートスレッドの割り込みハンドラでローカルの SFENCE.VMA を実行し、最後に
4) 操作が完了したことを発信元のスレッドにシグナルバックする、という方法があります。
これはもちろん、RISC-VのTLBシュートダウンと同じです。
\end{commentary}
...
変換データ構造が1つのアドレスマッピング(1ページまたはスーパーページ)に対してのみ変更されている場合、
{\em rs1}はそのマッピング内の仮想アドレスを指定して、
そのマッピングに対してのみ変換フェンスを作用させることができます。
さらに、変換データ構造が単一のアドレス空間IDに対してのみ変更されている一般的なケースでは、{\em rs2}がそのアドレス空間を指定できます。
SFENCE.VMA の動作は{\em rs1} と {\em rs2} によって以下のように異なります。
...
\begin{itemize}
\item {\em rs1}={\tt x0} かつ {\em rs2}={\em x0} の場合、フェンスはすべてのアドレス空間について、
  ページテーブルのどのレベルに対しても、すべてのリードとライトを順序付けます。
\item {\em rs1}={\tt x0} および{\em rs2} $\neq$ {\tt x0} の場合、
  フェンスはページテーブルのどのレベルに対してもすべてのリードとライトを順序づけますが、
  整数レジスタ{\em rs2} によって識別されたアドレス空間に対しての順序付けます。
  グローバルマッピング(~\ref{sec:translations}を参照)へのアクセスは命令されません。
\item {\em rs1}$\neq${\tt x0} かつ {\em rs2}={\tt x0}の場合、フェンスはすべてのアドレス空間について、
  {\em rs1}の仮想アドレスに関連するリーフページテーブルエントリへのリードとライトのみを順序付けします。
\item {\em rs1}$\neq${\tt x0} かつ {\em rs2}$\neq$ {\tt x0}の場合、フェンスは、
  整数レジスタ{\em rs2} で特定されるアドレス空間について、
  {\em rs1}の仮想アドレスに相関するリーフページテーブルエントリへの読み取りと書き込みのみを命令します。
  グローバル・マッピングへのアクセスは命令されません。
\end{itemize}
...
{\em rs2}$\neq${\tt x0} のとき、{\em rs2} に保持されている値のビットSXLEN-1:ASIDMAXは将来の使用のために予約されており、
ソフトウェアによってゼロにされ、現在の実装では無視されるべきです。
さらに、ASIDLEN ~$<$~ASIDMAX の場合、実装では{\em rs2} に保持されている
値のビットASIDMAX-1:ASIDLENを無視しなければならない。
...
\end{comment}
より単純な実装では、{\em rs1}の仮想アドレスと{\em rs2}のASID値を無視して、
常にグローバルフェンスを実行することができます。
\end{commentary}
...
\end{comment}
実装では、現在の{\tt satp}レジスタが指す変換データ構造の暗黙的な読み取りを、任意の時期に投機的に行うことができます。
これらの読み取りの結果は、インコヒーレント・キャッシュに保持されるかもしれませんが、他のhartとは共有されません。
キャッシュ・エントリは、{\tt satp}レジスタに現在読み込まれているASIDに対してのみ、またはグローバル・エントリに対してのみ確立することができます。
キャッシュは、{\tt satp} に現在読み込まれている ASID に対して確立されたエントリ、またはグローバルエントリに対してのみ、
暗黙のリードを満たすことができます。
{\tt satp}レジスタの変更は、そのような変換キャッシュを必ずしもフラッシュしません。
暗黙のリードが同じメモリロケーションへのライトを確実に守るためには、ライトの後にSFENCE.VMA命令を実行して、
関連するキャッシュされた変換をフラッシュしなければなりません。
...
\end{comment}
この仕様の結果、実装は、そのアドレスを包含する最新のSFENCE.VMA以降の任意の時点で有効だったアドレスの変換に使用することができます。
特に、リーフPTEが変更されても、それを包含するSFENCE.VMAが実行されない場合、
古い変換と新しい変換のどちらかが使用されますが、その選択は予測できません。
それ以外の動作は明確に定義されています。
...
defined.
\end{comment}
従来のTLB設計では、例えば、元の非リーフPTEのvalidビットをクリアせずに、
SFENCE.VMAを{\tt rs1}={\tt x0}で実行して、ページをスーパーページにアップグレードした場合、
複数のエントリが1つのアドレスに一致することがあります。
この場合、古いノンリーフPTEと新しいリーフPTEのどちらが使用されるかは予測できませんが、
それ以外の動作はよく定義されています。
\end{commentary}
...
\end{comment}
この仕様では、V(Valid)ビットがクリアされているPTEのキャッシュを許可しています。
オペレーティングシステムはこの可能性に対処するように書かれていなければなりませんが、
実装者は、無効なPTEを熱心にキャッシュすると、追加のページフォールトが発生してパフォーマンスが低下することに注意してください。
\end{commentary}
...
\end{comment}
実装では、現在の {\tt satp} レジスタの内容、またはそれに続く有効な (V=1) 変換データ構造エントリが
指す変換データ構造の暗黙的な読み取りのみを実行しなければならず、
暗黙的なアクセスについては、投機的に実行するのではなく、
命令実行の結果として生成されたものについてのみ例外を発生させなければなりません。
...
{\tt sstatus}のフィールドSUMとMXRの変更は、SFENCE.VMA命令を実行しなくても、
すぐに反映されます。
また、MODE を Bare から他のモードに変更した場合も、
SFENCE.VMA 命令を実行することなく、直ちに反映されます。同様に、{\tt satp.ASID} の変更も即時反映されます。
...
\end{comment}
SFENCE.VMA命令を実行する必要があるのは、次のような状況です。
...
\item ASIDをリサイクル(別のページテーブルに再結合)する場合は、
  リサイクルされたASIDを使用して新しいページテーブルを指すように変更し、
  リサイクルされたASIDを設定した上でSFENCE.VMAを実行します。
  ただし、次にリサイクルされたASIDがロードされるときに、
  同時に新しいページテーブルがロードされることを条件とします。
...
\item ASIDが実装されていない場合や、常にASID 0を使用することを選択した場合は、
  毎回の書き込み後にSFENCE.VMAを実行し、{\em rs1}={\tt x0}とする必要があります。
  なお、グローバル変換が変更されていない場合は、ゼロを含むレジスタを設定することで、
  グローバル変換がフラッシュされないようにします。
...
\end{comment}
\item 非リーフPTEを変更する場合は、SFENCE.VMAを実行する際に、{\em rs1}={\tt x0}に設定する必要があります。
  トラバーサルパス上のPTEにGビットが設定されていた場合は、{\em rs2}を{\tt x0}とし、
  そうでない場合は、{\em rs2}に翻訳の修正を行うASIDを設定します。
...
\end{comment}
\item リーフPTEを変更する場合は、SFENCE.VMAを実行し、ページ内の仮想アドレスを設定します。
  トラバーサルパス上のPTEにGビットが設定されていた場合はGビットを、
  それ以外の場合はASIDを設定してください(ASIDは変換変更対象となります)。
...
\end{comment}
\item リーフPTEのパーミッションを増やしたり、無効なPTEを有効なリーフに変更したりする特殊なケースでは、
  ソフトウェアはSFENCE.VMAをレイジーに実行することを選択できます。
  PTE を変更した後、SFENCE.VMA を実行する前に、
  新しいパーミッションまたは古いパーミッションのいずれかが使用されます。
  後者の場合、ページフォルト例外が発生する可能性があり、
  その時、ソフトウェアは前の箇条書きに従ってSFENCE.VMAを実行する必要があります。
\end{itemize}
\end{commentary}
...
\end{comment}
hart がアドレス変換キャッシュを使用する場合、そのキャッシュはその hart にとってプライベートなものでなければなりません。
特に、ASID の意味は hart にとってローカルなものであり、ソフトウェアは同じ ASID を使って異なる
hart の異なるアドレス空間を参照することができます。
...
scalability.
\end{comment}
将来的には、ASIDをSEE全体でグローバルなものとして再定義することで、
共有変換キャッシュやブロードキャストTLBシュートダウンのハードウェアサポートなどのオプションが可能になります。
しかし、OSが進化して、新しいASID管理技術を使ってTLBシュートダウンの範囲を大幅に縮小するようになっても、
ローカルASID方式は、そのシンプルさと、より優れたスケーラビリティのために、
今後も魅力的であり続けると考えています。
\end{commentary}
