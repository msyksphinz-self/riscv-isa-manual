...
\end{comment}
この節では、RVWMO(~\ref{ch:memorymodel}章)について、よりインフォーマルな言葉と具体的な例を用いて説明します。
これらは、公理と保存されたプログラムの順序規則の意味と意図を明確にすることを目的としています。
この付録は、解説として扱われるべきであり、すべての規範的な内容は、~\ref{ch:memorymodel}章とISA仕様の残りの本体に記載されています。
~\ref{sec:memory:discrepancies}節には、現在知られているすべての矛盾点が記載されています。
その他の不一致は意図的なものではありません。
...
\end{comment}
メモリ一貫性モデルには、弱いものから強いものまで様々な種類があります。
弱いメモリモデルは、ハードウェア実装の柔軟性が高く、性能、ワットあたりの性能、
消費電力、スケーラビリティ、ハードウェア検証のオーバーヘッドなどの点で、強いモデルよりも優れていますが、
プログラミングモデルが複雑になります。
強いモデルは、よりシンプルなプログラミングモデルを提供しますが、その代償として、
パイプラインやメモリシステムで実行可能な(非投機的な)ハードウェア最適化に多くの制限が課せられます。
その結果、消費電力、エリアオーバーヘッド、検証の負担などの面でコストがかかります。
...
\end{comment}
RISC-Vは、リリースコンシステンシの一種であるRVWMOメモリモデルを選択しました。
これにより、RISC-Vはメモリモデルの両極端の間に位置することになります。
RVWMOメモリモデルにより、アーキテクトは、シンプルな実装、アグレッシブな実装、
より大規模なシステムの奥深くに組み込まれ、
複雑なメモリシステムの相互作用にさらされる実装など、さまざまな可能性を構築することができます。
また、RVWMOメモリモデルは、プログラミング言語のメモリモデルを高い性能でサポートするだけの強度を備えています。
...
\end{comment}
他のアーキテクチャからのコードの移植を容易にするために、ハードウェアの実装によっては、Ztso拡張機能の実装を選択する場合があります。
Ztso拡張機能は、デフォルトでより厳密なRVTSO順序セマンティクスを提供します。
RVWMO用に書かれたコードは、自動的かつ本質的にRVTSOと互換性がありますが、
RVTSOを前提として書かれたコードは、RVWMOの実装上で正しく動作することは保証されていません。
実際、ほとんどのRVWMOの実装は、RVTSOのみのバイナリの実行を単純に拒否するでしょうし、そうすべきです。
そのため、各実装は、RVTSOコードとの互換性を優先するか(例えば、x86からの移植を容易にするため)、
代わりにRVWMOを実装している他のRISC-Vコアとの互換性を優先するかを選択する必要があります。
RVWMOを実装している他のRISC-V機器との互換性を優先するかを選択します。
...
\end{comment}
RVWMO用に書かれたコードの中には、RVTSOでは冗長になるフェンスやメモリ順序のアノテーションがあるかもしれません。
RVWMOのデフォルトがZtsoの実装に課すコストは、その実装でnoopになるフェンス(FENCE~R.RWやFENCE~RW.Wなど)をフェッチするための増分です。
RVWMOのデフォルトがZtsoの実装に課すコストは、その実装ではno-opとなるフェンス(例えば、FENCE~R,RWとFENCE~RW,W)をフェッチするための増分です。
しかし、Ztso以外の実装との互換性を確保するためには、これらのフェンスをコード内に残しておく必要があります。

\begin{comment}
\section{Litmus Tests}\label{sec:litmustests}
\end{comment}
\section{リトマステスト}\label{sec:litmustests}
\begin{comment}
...
\end{comment}
本章では、メモリモデルのある特定の側面をテストしたり、強調したりするために設計された小さなプログラムである、{\em リトマステスト}を用いて説明します。
図~\ref{fig:litmus:sample}は、2つのhartを使ったリトマステストの例です。
ここでは、すべてのhartで{\tt s0}～{\tt s2}が同じ値に設定されており、{\tt s0}が{\tt x}、{\tt s1}が{\tt y}、{\tt s2}が{\tt z}と表示されたアドレスを保持しているものとします。
左側にリトマステストのコード、右側に有効または無効な実行結果を表示しています。
...
\end{comment}
\caption{リトマステストの例と1つの禁止された実行({\tt a0=1})}
...
\end{comment}
リトマステストは、特定の具体的な状況におけるメモリモデルの意味合いを理解するために使用されます。
例えば、図~\ref{fig:litmus:sample}のリトマステストでは、第1のhartの{\tt a0}の最終値は、実行時の各hartからの命令ストリームの動的インターリーブによって、2、4、5のいずれかになります。
しかし、この例では、hart 0の{\tt a0}の最終的な値が1や3になることはありません。直感的には、ロードが実行される時点では1の値は見えなくなり、ロードが実行される時点では3の値はまだ見えていません。
このテストや他の多くのテストを以下で分析します。
...
\begin{table}[h]
  ...
  \caption{A key for the litmus test diagrams drawn in this appendix}
  \end{comment}
  \caption{本付録で登場するリトマステストのダイアグラムのキーとなる要素}
  \label{tab:litmus:key}
\end{table}
